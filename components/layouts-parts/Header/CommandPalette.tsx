...
import commandNavLinks from '@/data/menus/commandNavLinks'
...
export default function CommandPalette() {
  ...
  const filterednavigation = query
    ? commandNavLinks.filter((link) => link.title.toLowerCase().includes(query.toLocaleLowerCase()))
    : commandNavLinks
  return (
    <>
      <motion.button
              ...
              {filterednavigation.length > 0 && (
                <Combobox.Options static className="max-h-30 overflow-y-auto py-4 text-sm">
                  {filterednavigation.map((link) => (
                    <Combobox.Option key={link.title} value={link.href}>
                      {({ active }) => (
                        <div
